...
// 1. write me a get route that gets me all of the names of the users and returns them in an array of strings
//      filter then send array of names
...
// 2. write me a get route that gives me the phone numbers of everyone with an age of 24
...
app.get("/filterNumbers/", (req, res) => {
    ...
    res.send([{phonenumber: result[0].phone}]);
});

// 3. write a get route that gives me back all users over age 21 and age 26 or under
//      return an array of objects
app.get("/getAges", (req,res)=>{
    const ages = people.filter(
        (person) => person.age >21 && person.age <= 26
    )
    res.send(ages)
})
// 4. write me a get route that gives me back all the users email addresses
//      filter then send array of objects
...
app.get("/compileYearEndReport", (req,res)=>{
    ...
    const totalBalance = (arr) =>{
        ...
        for(let index of arr){
            totals.push(index.balance)
            for(let ele of totals)
                ele.replace("$", "")
                total += ele
                return total
        }
    }
    YER.push(totalBalance(people))
    res.send(YER)
})
